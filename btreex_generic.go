...
// Package btreex implements in-memory B-Trees of arbitrary degree.
//
// btreex implements an in-memory B-Tree for use as an ordered data structure.
...
// FreeListG represents a free list of B-Tree nodes. By default each
...
// Clear removes all items from the B-Tree.  If addNodesToFreelist is true,
...
// FreeList represents a free list of B-Tree nodes. By default each
// BTree has its own FreeList, but multiple BTrees can share the same
// FreeList.
// Two Btrees using the same freelist are safe for concurrent write access.
